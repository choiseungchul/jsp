	...
	<div class="content">
		<div class="hospital_tab">
			<ul>
				...
				<li class="on"><a href="${contextPath}/___/renew/board/list/31">성형후게시판</a></li>
			</ul>
		</div><!-- hospital_tab -->
		...
		<%--페이징 처리 --%>
			...
			<div class="page_navi">
				<% if(startIdx+1 > pagePerCount) {
					...
					%>
				<a href="${contextPath }/___/renew/board/list/31?PN=<%=ltPnum %>&QR=<%=query %>" class="btn">&lt;&lt;</a>
				<% }
					int loop = pcount;
				...
				%>	
				<%if(pnum == a){ %>	
				<a href="${contextPath }/___/renew/board/list/31?PN=<%=pnum %>&QR=<%=query %>" class="on"><%=a+1 %></a>
				<%}else{ %>
				<a href="${contextPath }/___/renew/board/list/31?PN=<%=a %>&QR=<%=query %>"><%=a+1 %></a>
				<%}//else %>
				<% } %>
				<% if( startIdx+pagePerCount < pcount ) {
					int gtPnum = startIdx + pagePerCount;
				%>
				<a href="${contextPath }/___/renew/board/list/31?PN=<%=gtPnum %>&QR=<%=query %>" class="btn">&gt;&gt;</a>
				<%} %>
			</div>
			<%} else { %>
			<div class="page_navi">
