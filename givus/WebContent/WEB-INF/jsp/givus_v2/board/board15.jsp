...
<div class="wrapper clear">
	...
	<div class="content">
		<div class="hospital_tab">
			<ul>
				<li class="on"><a href="${contextPath}/___/renew/board/list/15">공지사항</a></li>
		...
		<div class="notice_board">
			<table cellpadding="0" cellspacing="0" class="notice_board_tb">
				...
				<tr style="display: none;">	
					<td class="n_content" colspan="4"><%=data.get("contents") %></td>
				</tr>
				<% } %>
				</tbody>
			</table>
		...
		<%--페이징 처리 --%>
			...
			<div class="page_navi">
				<% if(startIdx+1 > pagePerCount) {
					...
					%>
				<a href="${contextPath }/___/renew/board/list/15?PN=<%=ltPnum %>&QR=<%=query %>" class="btn">&lt;&lt;</a>
				<% }
					int loop = pcount;
				...
				%>	
				<%if(pnum == a){ %>	
				<a href="${contextPath }/___/renew/board/list/15?PN=<%=pnum %>&QR=<%=query %>" class="on"><%=a+1 %></a>
				<%}else{ %>
				<a href="${contextPath }/___/renew/board/list/15?PN=<%=a %>&QR=<%=query %>"><%=a+1 %></a>
				<%}//else %>
				<% } %>
				<% if( startIdx+pagePerCount < pcount ) {
					int gtPnum = startIdx + pagePerCount;
				%>
				<a href="${contextPath }/___/renew/board/list/15?PN=<%=gtPnum %>&QR=<%=query %>" class="btn">&gt;&gt;</a>
				<%} %>
			</div>
			<%} else { %>
			<div class="page_navi">
