...
function rv_srch_btn(){
	...
	url = '${contextPath }/___/renew/hospital/board_review/<%=hid%>/'+cate+'/1/<%=type%>?srch=' + $('#rv_search').val();
...
$(function(){
	...
	$('.rv_board_sort_sct').change(function(e){
		var va = $('.rv_board_sort_sct option:selected').val();
		if(va == 'M'){
			loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/1/<%=type %>?sorting=3<%if(query != null ) out.print("&srch=" + query);%>');
		}else if(va == 'F'){
			loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/1/<%=type %>?sorting=4<%if(query != null ) out.print("&srch=" + query);%>');
		}else{
			loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/1/<%=type %><%if(query != null ) out.print("?srch=" + query);%>');
		}
	});
});
</script>
<div class="category">
	<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/0/1/<%=type %>')" class="<% if(cateId == 0) out.print("on");%>">[전체]</a>
	<% 
		for(int i = 0 ; i < cate.size() ; i++){
	%>
	<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cate.get(i).get("id")%>/1/<%=type %>');" <% if( cate.get(i).get("id").toString().equals(String.valueOf(cateId))) out.print("class=\"on\"");%>>[<%=cate.get(i).get("name") %>]</a>
	<%} %>
</div>
...
<div class="review_board">
	<div class="rv_board_sort">
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/1/<%=type %><%if(query != null ) out.print("?srch=" + query);%>')">+ 최신순</a>
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/1/<%=type %>?sorting=1<%if(query != null ) out.print("&srch=" + query);%>')">+ 조회순</a>
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/1/<%=type %>?sorting=2<%if(query != null ) out.print("&srch=" + query);%>')">+ 추천순</a>
		<select class="rv_board_sort_sct">
			<option value="A">전체</option>
			<option value="M" <%if(sex.equals("M")) out.print("selected='selected'"); %>>남자</option>
			<option value="F" <%if(sex.equals("F")) out.print("selected='selected'"); %>>여자</option>
		</select>
	</div>
	<table cellpadding="0" cellspacing="0" class="review_board_tb">
		...
		<tr>
			...
			<td class="subject"><a href="javascript:loadPage('${contextPath }/___/renew/hospital/boardview_review/<%=data.get("id")%>?xpath=<%=xpath%>');">
			[<%=data.get("cname") %>]
			<%=subject %>
			<% if(data.get("commentCount") != null) {
			...
			<td class="fs11"><%=data.get("recommendCount") %></td>
		</tr>
		<%
			}//for
		}else{ %>
	...
	<div class="review_write">		
		...
		if(um != null){
			if(um.getEmail() != null){
		%>
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/boardrv/write?type=<%=type %>&hid=<%=hid %>');" class="btn_bg2">글 작성하기</a>
		<%		
			}else{
		%>
	...
	<div class="page_navi pd60">
		<% if(startIdx > pagePerCount) {
			...
			%>
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/<%=ltPnum%>/<%=type %>');" class="btn">&lt;&lt;</a>
		<% }
			int loop = pcount;
		...
		%>	
		<%if(pnum == a){ %>	
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=cateId%>/<%=pnum %>/<%=type %>');" class="on"><%=pnum %></a>
		<%}else{ %>
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/<%=a %>/<%=type %>');"><%=a%></a>
		<%}//else %>
		<% } %>
		<% if(startIdx+pagePerCount < pcount ) {
			int gtPnum = startIdx + pagePerCount;
		%>
		<a href="javascript:loadPage('${contextPath }/___/renew/hospital/board_review/<%=hid%>/<%=cateId%>/<%=gtPnum %>/<%=type %>');" class="btn">&gt;&gt;</a>
		<%} %>
	</div>
	<%} else { %>
	<div class="page_navi pd60">
