...
$(function() {
	...
	$( "#btn_submit").click( function() {
		...
		if ( hospitalId == ''){
			...
			$(".hint-hospitalId").html("병원명을 입력하세요.").show();
			$("#hospitalId").focus();
			return false;
		}
		$.blockUI({ 
...
<div class="submitting">
	<div class="submitting-msg">병원 연결신청 중입니다...</div>
</div>
<%-- input field form --%>
<!-- variable -->
...
${dispatcher.data.id}
<c:choose>
<c:when test="${dispatcher.data.id != null && dispatcher.data.id > 0}"><c:set var="action" value="modify"/></c:when>
<c:otherwise><c:set var="action" value="create"/></c:otherwise>
</c:choose>
<form:form id="form" enctype="multipart/form-data" action="${contextPath}/edit.do?fid=${dispatcher.function.id}&action=${action}&id=${dispatcher.data.id}&pageType=${pageType}" method="post">
			...
			<td width="830px" class="right_body" valign="top" align="center"> 	
				...
				<tr>
					<td class="body_top" width="700px" height="200px">
						<div class="desc_page">연결 신청할 병원이름을 대상병원 입력란에 입력하여 선택해 주세요. <br>신청 전에  
							<a href="${contextPath}/___/p/user/modify/h"><span class="desc_page_strong">연락처와 사업자 등록번호가 바르게 입력되었는지 먼저 확인 부탁드립니다.</span></a> 
								관리자가 신청 내용을 확인 후 승인이 완료된 뒤에는 병원 정보를 직접 수정할 권한이 부여됩니다.<br> 연결 신청할 병원이 검색되지 않을 때는 
							<a href="${contextPath}/___/p/inquiry"><span class="desc_page_strong">관리자에게 문의</span></a> 바랍니다.
							<br>병원 연결 신청이 완료된 후에는 연결할 병원을 변경할 수 없습니다.
						</div>
					</td>
				</tr>
				<tr>
					<td height="25px" class="right_subtitle"><img src="${imageHandler.g_right_subtitle_icon}"> 병원 연결 신청</td>
				</tr>
				<%-- //////////////// USER INFO TABLE //////////////// --%>
				<tr>
