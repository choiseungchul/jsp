...
$(function(){
	$('.message-list td[sendMessageId]').click(function(){
		...
		var url = '${contextPath}/___/msg/receive/view/' + sendMessageId;
		$('.message-contents').load( url, function( responseText){

		});
		$('.message-list tr').removeClass('message-selected');
		$(this).closest('tr').addClass('message-selected').find('.read').html('읽음');
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td align="center">
	...
	<table width="980" style="margin:auto;margin-top:20px;" >
		<tr>
			<td height="50px" class="path_info" align="left"><c:choose><c:when test="${ pageType=='m'}">마이페이지</c:when><c:otherwise>병원페이지</c:otherwise></c:choose> > 받은쪽지함 </td>
		</tr>
		</table>
	</td>
</tr>
		...
		<tr><%-- //////////////// LEFT MENU //////////////// --%>
			...
			<td width="830px" class="right_body" valign="top" align="center"> 				
				<table width="700px" border="0" cellspacing="0" cellpadding="0">
					...
					<div class="right_subtitle"><img src="${imageHandler.g_right_subtitle_icon}"> 받은쪽지함</div>
					<div class="message-list">
						<div style="height:545px;">
						<table>
							<tr>
								<td class="message-head" width="26px"><input type="checkbox" name="chkall" id="chkall"></td>
								<td class="message-head" width="133px">보낸사람</td>
								<td class="message-head" width="100px">날짜</td>
								<td class="message-head" width="50px">읽기여부</td>
							</tr>
							...
							<tr>
								...
								<td class="message-value" sendMessageId="${data.id}">${data.renderedData.sendUserId}</td>
								<td class="message-value">${data.renderedData.createDate}</td>
								<td class="message-value read">
									<c:if test="${data.readDate != null}">읽음</c:if>
									<c:if test="${data.readDate == null}">읽지않음</c:if>
								</td>
							</tr>
							</c:forEach>
						</table>
