  ...
  <div class="row">
	 <%@include file="../inc/left.jsp" %>	 
 	 <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <form action="${contextPath }/___/admin/board/modproc" method="post" id="frm" name="frm">
          ...
          <div class="table-responsive">          
              ...
              <thead>
				...
				<tr>
					...
					<td class="subject">
						<select name="category" id="category">							
							...
							%>
							<option value="<%=c_id%>" <%if(c_name.equals(data.get("cname").toString().trim())) out.print("selected='selected'"); %>><%=c_data.get("name") %></option>
							<%} %>
						</select>						
						<input type="text" name="subject" value="<%=data.get("subject").toString().trim() %>">					
					...
					<td class="tit">작성자</td>
					<td>
						<input type="text" name="creator" value="<%=data.get("creator") %>">
					</td>
				</tr>
				<%
					if(type.equals("25") || type.equals("26")){
				...
				<tr>
					...
					<td class="subject" colspan="3">
						<select name="ch_hid" id="ch_hid">
					<%
						for(int k = 0 ; k < hos.size(); k++){
							HashMap<String, Object> hosInfo = hos.get(k);
							String h_id = hosInfo.get("id").toString();
					%>
							<option value="<%=h_id%>" <%if(hid.equals(h_id)) out.print("selected='selected'"); %>><%=hosInfo.get("name")%></option>
					<% } %>
						</select>
					</td>
				</tr>
				...
				<tr>
					<td class="tit">날짜</td>
					<td class="subject">
						<input type="text" name="createDate" value="<%=data.get("createDate") %>">
					</td>
					<td class="tit">조회수</td>
					<td>
						<input type="text" name="viewCount" value="<%=data.get("viewCount") %>">
					</td>
				</tr>				
				<tr>
					<td colspan="4" class="contents_td">					
						<textarea rows="" cols="" id="ir1" name="contents" class="ir1"><%=data.get("contents") %></textarea>
					</td>
				</tr>
				...
				%>
					...
					<tr>
						...
						<td class="point">
							<input type="hidden" value="<%=review.get(i)%>">
							<div class="statbar"></div>				
						</td>
						<td class="point">
							<div class="stat_text"><%=MessageHandler.getMessage("reviewPoints.answer." + review.get(i).toString()) %></div>
						</td>
					</tr>
					<% } %>		
				...
				<tr>
					...
					<td class="subject">
						<select name="gender" id="gender">
							<option value="F" <%if(gender.equals("F")) out.print("selected='selected'"); %>>여자</option>
							<option value="M" <%if(gender.equals("M")) out.print("selected='selected'"); %>>남자</option>
						</select>
					</td>
					<td class="tit">리뷰포인트 적용 생일(연령대)</td>
					<td>
						<select id="age" name="age">
							<option value="10" <%if(age.equals("10")) out.print("selected='selected'"); %>>10대</option>
							<option value="20" <%if(age.equals("20")) out.print("selected='selected'"); %>>20대</option>
							<option value="30" <%if(age.equals("30")) out.print("selected='selected'"); %>>30대</option>
							<option value="40" <%if(age.equals("40")) out.print("selected='selected'"); %>>40대</option>
						</select>
					</td>
				</tr>
				<% } %>
			...
			<div class="board_btns " style="padding-bottom: 50px;">
				<button type="button" class="btn btn-default btn-lg"  onclick="send_mod()">수정</button>
